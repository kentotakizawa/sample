...
def happiness_estimation_pre(rules_df):
  ...
  return rules_df_rules, confidence_list


#推定幸福度を計算する
def happiness_estimation(profile_df, rules_df, rules_df_rules, dictionary, confidence_list):
...
def calculate_rmse(rules):
  ...
  for index in range(len(profiles_df)):
    estimated_happiness = happiness_estimation(profiles_df.iloc[index, :].to_frame().T, rules_df, rules_df_rules, dictionary, confidence_list)
    correct_happiness = profiles_df.iloc[index, :].to_frame().T['本人の幸福感（最近１年間）'].iloc[0]
    squared_error = (estimated_happiness - correct_happiness) ** 2
    squared_errors.append(squared_error)
  rmse = np.sqrt( np.nanmean(squared_errors) )
